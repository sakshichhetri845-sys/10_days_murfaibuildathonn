...
class MemoryService:
    ...
    def get_memory(user_id: str) -> Optional[dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error reading memory for user_id={user_id}: {e}", exc_info=True)
            return None
        finally:
            conn.close()
    ...
    def save_memory(
        ...
        try:
            ...
            logger.error(f"Error saving memory for user_id={user_id}: {e}", exc_info=True)
            return False
        finally:
            conn.close()
    ...
    def delete_memory(user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error deleting memory for user_id={user_id}: {e}", exc_info=True)
            return False
        finally:
            conn.close()
    ...
    def format_memory_summary(memory: Optional[dict[str, Any]]) -> str:
        ...
        if memory.get("ongoing_conditions"):
            parts.append(f"Ongoing conditions (consented): {', '.join(memory['ongoing_conditions'])}")
