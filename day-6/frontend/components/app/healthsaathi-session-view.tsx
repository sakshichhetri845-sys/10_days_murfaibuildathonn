...
export function HealthSaathiSessionView({ onBackToLanding }: HealthSaathiSessionViewProps) {
  ...
  // Outbound Session Detection based on room name
  const roomName = room?.name || '';
  const isOutbound = roomName.startsWith('outbound-') || roomName.startsWith('followup-');
        ...
        >
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm leading-relaxed text-slate-600">
              HealthSaathi needs access to your microphone so it can hear your responses during the
              follow-up call.
            </p>
  ...
  // ACTIVE VOICE INTERFACE (INBOUND AND OUTBOUND MODES)
  return (
      ...
      <header className="sticky top-0 z-30 flex items-center justify-between border-b border-teal-100/70 bg-[#F0F7FF]/85 px-4 py-3.5 backdrop-blur-md sm:px-8">
        <div className="flex items-center gap-3">
          <div className="flex size-10 items-center justify-center rounded-2xl bg-gradient-to-br from-teal-600 to-sky-600 text-white shadow-sm">
            {isOutbound ? <PhoneOutgoing className="size-5" /> : <Activity className="size-5" />}
          </div>
          <div>
            <div className="flex items-center gap-2">
              ...
              >
                {isOutbound ? 'Outbound Follow-up' : 'Voice Session'}
              </span>
            </div>
            <p className="hidden text-xs text-slate-500 sm:block">
              {isOutbound
                ? 'HealthSaathi is following up on your previous health inquiry'
        ...
        <div className="flex items-center gap-3">
          ...
          <Button
            ...
            size="sm"
            className="flex items-center gap-1.5 rounded-full bg-rose-700 px-4 py-1.5 text-xs font-semibold text-white shadow-sm transition-colors hover:bg-rose-800"
          >
            <PhoneOff className="size-3.5" />
            <span>{isOutbound ? 'End Follow-up Call' : 'End Session'}</span>
          </Button>
        </div>
      </header>
      ...
      <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col justify-between gap-6 px-4 py-6 sm:px-6">
        ...
        <div className="relative flex flex-1 flex-col items-center justify-center py-6">
          <div className="w-full max-w-lg space-y-6 rounded-3xl border border-teal-100/90 bg-white/90 p-8 text-center shadow-xl shadow-teal-900/5 backdrop-blur-sm">
            {/* Header Emblem */}
            <div
              className={`mx-auto flex size-14 items-center justify-center rounded-2xl border ${
                isOutbound
            ...
            >
              {isOutbound ? (
                <PhoneCall className="size-7 text-sky-600" />
              ) : (
                <Heart className="size-7 fill-teal-600/20 text-teal-600" />
              )}
            </div>

            {/* Dynamic Outbound vs Inbound State Headlines */}
            {isOutbound ? (
              // DAY 6 OUTBOUND UI STATES
              currentStatus === 'speaking' ? (
                <div className="space-y-2">
                  ...
                  <h2 className="text-2xl font-extrabold text-slate-900">
                    Following up on your previous inquiry...
                  </h2>
                  <p className="text-xs text-slate-500">
                    HealthSaathi is explaining your follow-up details in simple language.
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <h2 className="text-2xl font-extrabold text-slate-900">
                    Take your time. How are you feeling today?
                  </h2>
                  <p className="text-xs text-slate-500">
                    You can speak naturally in English, Hindi, or Hinglish.
                  </p>
                </div>
              ) : (
                <div className="space-y-2">
                  ...
                  <h2 className="text-2xl font-extrabold text-slate-900">
                    HealthSaathi Follow-up Call
                  </h2>
                  <p className="text-xs text-slate-500">
                    You can end the call at any time using the End Call button below.
                  </p>
                </div>
              )
            ) : // DAYS 1–5 INBOUND UI STATES
            currentStatus === 'speaking' ? (
              <div className="space-y-2">
                <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-100 bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700">
                  <span className="size-2 animate-pulse rounded-full bg-emerald-500" />
              ...
              {currentStatus === 'speaking' ? (
                <div className="flex items-center gap-1.5">
                  {[0.5, 1, 0.7, 1.2, 0.6, 0.9, 0.4].map((h, i) => (
                    <motion.div
                      ...
                      className={`w-1.5 rounded-full ${isOutbound ? 'bg-sky-600' : 'bg-teal-600'}`}
                    />
                  ))}
                </div>
              ) : currentStatus === 'listening' ? (
                ...
                <div className="flex items-center justify-center pt-2">
                  <Button
                    ...
                    className={`rounded-full px-6 py-2.5 text-sm font-bold text-white shadow-md ${
                      isOutbound ? 'bg-sky-700 hover:bg-sky-800' : 'bg-teal-700 hover:bg-teal-800'
                    }`}
                  >
                    Start Talking
                  </Button>
              ...
              {messages.length === 0 ? (
                <div className="py-4 text-center text-xs text-slate-500">
                  <p className="font-semibold text-slate-700">
                    &quot;
                    {isOutbound
                      ? 'Namaste! HealthSaathi follow-up call connected.'
                      : "Namaste! I'm HealthSaathi."}
                    &quot;
                  </p>
                  <p className="mt-0.5">
                    {isOutbound
                      ? 'HealthSaathi is starting the follow-up conversation.'
        ...
        <div className="flex items-center justify-center rounded-2xl border border-teal-100 bg-teal-50/60 p-3 text-center text-xs text-teal-900">
          <p className="text-[11px] font-medium">
            HealthSaathi provides general educational follow-up information. It does not replace
        ...
        <div className="flex items-center justify-center gap-4 py-2">
          ...
          <Button
            onClick={handleEndCall}
            size="lg"
            className="flex size-14 items-center justify-center rounded-full bg-rose-700 text-white shadow-md transition-colors hover:bg-rose-800"
