...
export function HealthSathiSessionView() {
  ...
  const { state: voiceState } = voiceAssistant;
  ...
  // Active Specialist Agent Detection & Attributes
  const remoteParticipant = Array.from(room?.remoteParticipants?.values?.() ?? []).find(
    (p) => p.attributes?.agent_name
  );
  const agentAttr = remoteParticipant?.attributes || voiceAssistant.agent?.attributes || {};

  const isClinicSathi =
    agentAttr.agent_name === 'ClinicSathi' ||
    messages.some(
      (m) =>
        !m.from?.isLocal &&
        (m.message?.includes('Connecting to ClinicSathi') ||
          m.message?.includes("I'm ClinicSathi") ||
          m.message?.includes('ClinicSathi'))
    );

  const activeAgentName = isClinicSathi ? 'ClinicSathi' : agentAttr.agent_name || 'HealthSathi';
  const activeAgentTitle = isClinicSathi
    ? 'Healthcare Facility Specialist'
    : agentAttr.agent_title || 'General Health Support';
  const activeVoiceName = isClinicSathi
    ? agentAttr.voice_name || 'Murf Falcon · Pooja'
    : agentAttr.voice_name || 'Murf Falcon · Samar';

  // Track handoff transitions
  useEffect(() => {
    const lastMsg = [...messages].reverse().find((m) => !m.from?.isLocal)?.message ?? '';
    if (
      lastMsg.includes('Connecting to ClinicSathi') ||
      lastMsg.includes('transfer_to_clinic_sathi')
    ) {
      setTransitionBanner('Connecting to ClinicSathi...');
      const timer = setTimeout(() => {
        setTransitionBanner('ClinicSathi is now helping');
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#F0FAFA] font-sans text-slate-900">
      ...
      <header className="sticky top-0 z-30 border-b border-teal-100/80 bg-[#F0FAFA]/90 px-4 py-4 backdrop-blur-md sm:px-8">
        <div className="mx-auto flex max-w-3xl items-center justify-between">
          <div className="flex items-center gap-3">
            <div
              className={cn(
                'flex size-10 items-center justify-center rounded-xl text-white shadow-md transition-all duration-300',
                isClinicSathi
                  ? 'bg-gradient-to-br from-teal-600 to-cyan-600 shadow-cyan-600/30'
            ...
            <div>
              ...
              <p className="text-[11px] font-medium text-teal-700">{activeAgentTitle}</p>
            </div>
          </div>
      ...
      <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-6 px-4 py-8 sm:px-8">
        {/* Specialist Handoff Transition Banner */}
        <AnimatePresence>
          {transitionBanner && (
            <motion.div
              ...
              className={cn(
                'mx-auto flex items-center gap-2 rounded-full px-4 py-1.5 text-xs font-bold shadow-sm transition-all',
                transitionBanner.includes('Connecting')
                  ? 'border border-amber-200 bg-amber-50 text-amber-800'
                  : 'border border-teal-200 bg-teal-50 text-teal-800'
              )}
            >
              <Sparkles className="size-3.5 animate-pulse text-teal-600" />
              <span>{transitionBanner}</span>
            </motion.div>
          )}
        </AnimatePresence>
        {/* Voice State Orb */}
