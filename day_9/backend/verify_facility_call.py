...
from db import get_call_analytics, get_nearby_facilities, init_db, save_call_analytics
...
def verify_live_facility_logging():
    ...
    for call in analytics["recent_calls"][:3]:
        print(
            f" - ID: {call['session_id']} | Outcome: {call['outcome']} ({call['outcome_type']}) | Duration: {call['duration']}s"
        )
