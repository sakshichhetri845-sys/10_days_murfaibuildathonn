...
async def lookup_user_memory(
    ...
    memory_info = {
        ...
        "reminder_preference": facts.get("reminder_preference")
        or facts.get("learning_goal"),
        "contact_preference": facts.get("contact_preference"),
        "interaction_preferences": facts.get("interaction_preferences")
        or facts.get("topics_practiced", []),
    }
...
async def save_user_memory(
    ...
    clean_reminder = (
        (reminder_preference or learning_goal or level).strip()
        if (reminder_preference or learning_goal or level)
        else None
    )
    clean_contact = contact_preference.strip() if contact_preference else None
    clean_interaction = (
        (topic_practiced or recurring_challenge).strip()
        if (topic_practiced or recurring_challenge)
        else None
    )
