    ...
    async with (
        ...
        for ev in turn1.events:
            ...
            if item and getattr(item, "name", "") == "create_escalation":
                pytest.fail(
                    "create_escalation tool was invoked before consent was granted!"
                )

        turn2 = await session.run(
            user_input="No, I do not want to create a support request."
        )
...
async def test_test_e_consent_yes_executes_escalation(temp_db) -> None:
    ...
    async with (
        ...
        await session.start(Assistant())
        await session.run(
            user_input="Can you submit a human health support request for me?"
        )

        result = await session.run(
            user_input="Yes, I confirm. Please execute create_escalation to submit my support request now."
        )
