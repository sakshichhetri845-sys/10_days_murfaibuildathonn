...
def update_current_call_outcome(
    ...
    try:
        ...
        if not call_state and hasattr(context, "proc") and hasattr(context.proc, "userdata"):
        ...
        if call_state:
            ...
            if new_level >= current_level or outcome == "failed":
                ...
                try:
                    import time
                    duration = max(1, int(time.time() - call_state.get("start_time", time.time())))
                    save_call_analytics(
...
class Assistant(Agent):
    ...
    async def find_nearby_facility(
        ...
        Parameters:
            location: User's area, city, or district (e.g. 'Pune', 'Nagpur', 'Mumbai', 'Delhi', 'Kathmandu').
        ...
        try:
            ...
            # Query database memory for nearby facilities in Pune, Nagpur, Mumbai, Delhi, etc.
            ...
            location_note = (
                f"In or near {loc_clean}: " if loc_clean else ""
            )
            ...
            result = {
                ...
                "disclaimer": (
                    "HealthSathi provides facility information for guidance. "
            ...
            logger.error(f"TOOL ERROR in find_nearby_facility: {e}")
            update_current_call_outcome(context, "TECHNICAL_ERROR", "failed")
            raise
...
async def my_agent(ctx: JobContext):
    ...
    session_id = room_name or getattr(ctx.room, "sid", "") or f"call_{int(start_time)}"
    ...
    if "sched_task" not in ctx.proc.userdata:
        try:
            ...
            ctx.proc.userdata["sched_task"] = asyncio.create_task(start_scheduler_loop())
    ...
    try:
        ...
        if (call_state.get("user_spoke") or duration > 3) and call_state.get("outcome") != "successful":
            call_state["outcome"] = "successful"
            if call_state.get("outcome_type") == "INCOMPLETE":
                call_state["outcome_type"] = "GENERAL_GUIDANCE"
        try:
